...
- After they accept a voice note (or a patch to it), write `07-record/author-voice.md` (or `paths.author_voice`). Do not invent the note. After they accept manuscript wording from **Draft this in my voice** or **Edit this in my voice**, write that wording into the named section file (`_*.qmd` when includes are in use; `paper.qmd` if that section still lives there). Title and abstract stay in `paper.qmd` YAML.
