...
- Records live under `layout.yml` path `source_records` (default `07-record/sources/`). Optional PDF copies live under path `sources` (default `sources/`). Those PDFs are copies for sharing and NotebookLM in **this** Google Drive folder. Do not create a second Drive library. Do not rename files inside Zotero.
...
1. List items in the linked Zotero collection if a Zotero connector or local Zotero is available. If not, work from `references.bib` plus any PDFs already in `sources/`, and say that the live Zotero collection was not read.
...
4. **PDF copies** (only if they asked, or they already use `sources/` for NotebookLM): copy the Zotero attachment into `sources/` with a name starting with the item key:
...
- Write to Zotero
- Treat `sources/` as canonical instead of Zotero
