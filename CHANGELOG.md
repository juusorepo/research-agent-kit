...
A new paper includes a thin `CLAUDE.md` that points at `AGENTS.md`, so Claude Code can start without a hand-written briefing. It is not a second rule set. Older papers get the same file if it is missing when you **Make this paper self-contained** or **Adjust this project**. Shared conventions for several papers can live in a parent-folder `CLAUDE.md`.
...
An audit writes a hash list of files it actually read, keeps a required “what could not be checked” section, and does not narrow the scope the researcher named. It still writes only the report, at most one task row, and an AI-use note if that box is ticked. Checking a literature statement against a source adds a row to `claim-checks.md` (citation key, manuscript location, source, what was verified). That is per-statement; the AI-use `check:` field remains the whole run.
