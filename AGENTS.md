...
If they say **Start the project** or **Initiate**: use `skills/start-research-project/SKILL.md` and `policies/how-to-talk.md`. Ask the interview questions (with defaults) and wait. After they reply, copy the paper skeleton and patch; do not read template files. If there is no local kit and they gave a GitHub URL, fetch a temporary copy, write a self-contained paper into the open folder, delete the temporary copy. Understand the project is the next message. Default folder **paper-1** only if Start runs inside the kit. The skeleton includes a thin `CLAUDE.md` that points at `AGENTS.md`; do not replace it with a second rule set. Do not require Python or R.
