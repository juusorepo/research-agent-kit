...
If they say **Start the project** or **Initiate**: use `skills/start-research-project/SKILL.md` and `policies/how-to-talk.md`. Ask the interview questions (with defaults) and wait. If they have not already opened a paper folder or given a path, question 1 is **where** to create it (default **paper-1** next to the kit). If this folder is the paper and they named a local kit, read the start skill from that kit and write only here — do not fetch GitHub. After they reply, copy the paper skeleton and patch; do not read template files. If there is no local kit and they gave a GitHub URL, fetch a temporary copy, write a self-contained paper into the open folder, delete the temporary copy. Understand the project is the next message. Do not copy `CLAUDE.md`. Do not require Python or R.
