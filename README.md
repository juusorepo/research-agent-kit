...
| Skill | What it does |
|---|---|
| Start the project | Create a paper folder that follows the kit. Codex: open the paper folder and name the **local kit** path (no GitHub). Cursor: open kit and paper, then the phrase is enough. GitHub only if there is no local kit |
...
| Sync the bibliography · Can you read Zotero · Link the Zotero collection | Refresh this paper’s source list from a Zotero collection on this computer, or from `references.bib`. The check uses curl. Citation keys still come from that export. Copies for sharing stay in `08-sources/` (Zotero stays canonical) |
...
**Start the project:** create an empty paper folder, open it, and paste **Start the project** with the path to your local kit (Codex). Cursor can open the kit and the paper together and just say **Start the project**. GitHub is only if you have no kit folder: **Start the project from** the GitHub URL. If you have not already opened a paper folder or given a path, the assistant asks **where** to create it (default **paper-1** next to the kit). The assistant asks the interview questions (with defaults) and waits; after the answers, setup is copy-then-patch. Understand the project is the next message. Generated kit files are replaced from GitHub with **Update the kit** in that paper, or from a local kit with **Adjust this project**. Do not edit them. How it talks is in `policies/how-to-talk.md`.
...
Small extra setup may be needed for a specific tool. That setup lives in the kit `adapters/` folder and only *points* at these files (Stata, Drive, Docs, NotebookLM, Claude Code, Codex, and an optional manuscript review service). On **Windows**, a Google Drive paper should be opened from the local mirrored folder, not `G:\My Drive` — see `adapters/google-drive/`. Papers use `AGENTS.md`. Claude Code does not load that file unless you add a one-line `CLAUDE.md` (`@AGENTS.md`) — see `adapters/claude/`. Codex reads `AGENTS.md`; to start in an empty paper folder, name the local kit — see `START.md` and `adapters/codex/`.
