...
**Independent check.** Open a **new chat** on **this computer** and say **Audit the research chain** (full chain, or one link). Large artifacts and hashes need the files here; a cloud session that has not transferred them cannot audit them. You can say this in a kit paper **or** in a folder that was never set up with this kit. If there is no folder map, the assistant asks where the plan, code, outputs, and manuscript are (with defaults if it can see them) and waits once. It does not create a kit paper. Missing pieces are recorded as not checked, not as a pass. The saved report lists SHA-256 of files actually read and what could not be checked.
