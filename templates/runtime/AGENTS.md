...
Do not load `notes/` by default. Files in `contributions/` are proposals, not agreed analyses. A research packet and evidential-status notes are drafts unless an approved result says otherwise. They do not override the analysis plan, accepted decisions, or approved results.
...
- Restricted or confidential files must not be uploaded to NotebookLM or other external source tools.
